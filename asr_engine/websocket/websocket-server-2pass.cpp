...
nlohmann::json handle_result(FUNASR_RESULT result, std::string& online_res,
    ...
    jsonresult["wav_name"] = msg;

    if (tmp_online_msg != "") {
      LOG(INFO) << msg << " online_res :" << tmp_online_msg;
    ...
    if (tmp_tpass_msg != "") {
      LOG(INFO) << msg << " offline results : " << tmp_tpass_msg;
    ...
    case websocketpp::frame::opcode::text: {
      ...
      if (jsonresult.contains("chunk_size")){
        if(msg_data->tpass_online_handle == NULL){
          ...
          if (jsonresult["chunk_size"] == 10) {
            chunk_size_vec = {5, 10, 5};
          } else if (jsonresult["chunk_size"] == 8) {
            chunk_size_vec = {8, 8, 4};
          } else {
            LOG(INFO) << "invalid chunck_size setting ...";
          }
          FUNASR_HANDLE tpass_online_handle =
